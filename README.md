...
```python
# IVF 인덱스 (대규모 데이터)
--kb-faiss-k 500   # Stage 1 후보 (속도 vs 정확도)
--faiss-k 900      # Feedback 후보

# GPU 가속 (10~100배 속도 향상)
--kb-use-gpu --fb-use-gpu
```

### 2. 캐시 시스템
```bash
# 첫 실행: FAISS 인덱스 구축 (~30초)
# 이후 실행: 캐시 로드 (~2초)
...
### 3. 배치 처리
```python
# 1000개 쿼리 → FAISS 1번 호출
# Vectorized SHAP One-Hot 인코딩 (100배 속도 향상)
```

### 4. 벤치마크
| 작업 | 데이터 크기 | CPU | GPU | 속도 향상 |
|------|------------|-----|-----|----------|
| KB 인덱스 구축 | 4.2M | 45초 | 5초 | 9배 |
| KB 적용 | 1000개 | 120초 | 15초 | 8배 |
| Feedback 적용 | 500개 | 60초 | 8초 | 7.5배 |
...
### Q2. GPU 메모리 부족
```python
# GPU 메모리 설정 (tools/knowledge_base.py)
self.gpu_resources.setTempMemory(512 * 1024 * 1024)  # 512MB로 감소
```
...
```bash
# 라운드별 개별 실행
for round in Round_1 Round_2 Round_3; do
    python pipeline_orchestrator_v2.py --rounds $round
done
```
...
### 정확도 향상 추이
```
Detection Only      → 베이스라인 (예: 92%)
+ Train KB          → ~95% (4.2M 케이스 활용)
+ Feedback (Round2) → ~97% (동적 증축)
+ Feedback (Round3) → ~98% (누적 증축)
```

### 처리 속도 (GPU)
```
Detection:       ~500 cases/sec
Prioritizer:     ~1000 cases/sec
KB Apply:        ~60 cases/sec (FAISS)
Feedback Apply:  ~50 cases/sec (FAISS)
```
...
- **프로젝트**: [GitHub Repository](https://github.com/your-repo/feed-nids)

---

## 🙏 감사의 말

- **FAISS**: Facebook AI Research의 고성능 벡터 검색 라이브러리
- **XGBoost**: 효율적인 그래디언트 부스팅 프레임워크
- **SHAP**: 설명 가능한 AI를 위한 모델 해석 도구
